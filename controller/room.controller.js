...
async function addroom(req, res) {
  ...
  const { room_number, room_level, room_capacity, room_type, facilities_id } =
    req.body;
  ...
  if (areNumbersInvalid) {
    return res
      .status(400)
      .json({ message: "Room Capacity and Room Level must be numbers" });
  }

  try {
    const roomNumberCheckQuery = `SELECT room_number FROM rooms WHERE room_number = $1`;
    const roomNumberCheckResult = await client.query(roomNumberCheckQuery, [
      room_number,
    ]);
    ...
    logging(
      ...
      userId,
      "Success",
      `Room addition successful, room_id: ${roomId}`
    );
...
async function updateroom(req, res) {
  ...
  const areNumbersInvalid =
    isNaN(roomId) || isNaN(room_capacity) || isNaN(room_level);
  if (areNumbersInvalid) {
    return res
      .status(400)
      .json({
        message: "Room ID, Room Capacity, and Room Level must be numbers",
      });
  }
  ...
  try {
    ...
    logging(
      ...
      userId,
      "Success",
      `Room update successful, room_id: ${roomId}`
    );
...
async function deleteroom(req, res) {
  ...
  try {
    ...
    logging(
      ...
      userId,
      "Success",
      `Room deletion successful, room_id: ${room_id}`
    );
